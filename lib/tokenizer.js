...
function Tokenizer(input, options = {}) {
    ...
    while (position < inputLength) {
        ...
        if (token.type === K.TOKEN_INCLUDE) {
            tokens = tokens.concat(
                Tokenizer(fs.readFileSync(token.value, 'utf8'), options)
            )
        } else {
            tokens.push(token)
        }
    ...
    function getNextPosition(currentType) {
        switch (currentType) {
            ...
            case K.TOKEN_INCLUDE: {
                return input.indexOf(options.includeEndTag, position + options.includeStartTag.length) + options.includeEndTag.length
            }
            default: {
